...
// Configuration for void rescue functionality
interface VoidRescueConfig {
  enabled: boolean; // Whether the void rescue feature is enabled
  consumeHelmet: boolean; // Whether to consume the phase head item
  absorptionAmount: number; // Amount of absorption hearts to grant
  absorptionDuration: number; // Duration of absorption effect in seconds
  rescueCooldown: number; // Cooldown between rescues in seconds
  totemEffectDuration: number; // Duration of totem effects in seconds
  debugMessages: boolean; // Whether to display debug messages
}

// Default configuration
const DEFAULT_CONFIG: VoidRescueConfig = {
  enabled: true,
...
let config: VoidRescueConfig = { ...DEFAULT_CONFIG };

// Store player rescue cooldowns
const playerRescueCooldowns = new Map<string, number>();

// Store the phase head ID
let phaseHeadId: string = "";

/**
 * Checks if a player is wearing the phase head
 * @param player The player to check
 */
function isWearingPhaseHead(player: Player): boolean {
  try {
 ...
 */
function consumePhaseHead(player: Player): boolean {
  try {
 ...
 */
function applyTotemEffects(player: Player): void {
  try {
 ...
 */
function teleportToSpawnPoint(player: Player): boolean {
  try {
 ...
 */
function handleEntityDamage(event: any): void {
  // Skip if disabled
  if (!config.enabled) return;

  try {
    // Check if damage is to a player
    if (!event.hurtEntity?.id || !(event.hurtEntity instanceof Player)) {
      return;
    }
 ...
 */
export function startVoidRescueSystem(headId: string, customConfig?: Partial<VoidRescueConfig>): void {
  // Store the phase head ID
  phaseHeadId = headId;
 ...
 */
export function stopVoidRescueSystem(): void {
  // Clear cooldown data
 ...
 */
export function updateVoidRescueConfig(newConfig: Partial<VoidRescueConfig>): void {

...
function hasSkyAccess(player) {
    try {
        ...
        for (let y = 0; location.y + y <= 256; y++) {
            ...
            if (!(blockAbove === null || blockAbove === void 0 ? void 0 : blockAbove.isAir)) {
                return false;
            }
        }
        return true;
...
export function startPhantomSpawner(customConfig) {
    ...
    const intervalId = system.runInterval(() => {
        ...
        for (const player of players) {
            ...
            if (config.debugMode) {
                player.sendMessage(`§7Insomnia Days: ${insomniaDays}, Above Min Y: ${aboveMinY ? "§a✓" : "§c✗"}, ` +
            ...
            if (config.debugMode) {
                player.sendMessage(`§7Phantom Spawn Roll: ${spawnSuccessful ? "§aSuccess" : "§cFail"} ` +
            ...
            if (!aboveMinY || !skyAccessible) {
                ...
                player.sendMessage(`§e⚠ Phantom spawn blocked! Counter: ${playerData.blockedSpawns}/${config.blockedSpawnsBeforePhase}`);
    ...
    }, config.testingMode ? 20 : 100); // Run more frequently in testing mode
...
//# sourceMappingURL=phase-spawner-template.js.map
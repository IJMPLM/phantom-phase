    ...
    for (const player of players) {
        ...
        if (playerData.lastDayChecked < currentDay) {
            ...
            if (playerSlept) {
                ...
                if (config.debugMode) {
                    player.sendMessage(`§bYou did not sleep! Days since last rest incremented to ${playerData.daysSinceLastRest}`);
                }
            }
        }
            ...
            for (let i = players.length - 1; i >= 0; i--) {
                ...
                if (belowMinY || noSkyAccess) {
                    ...
                    if (spawnSuccessful) {
                        ...
                        if (config.debugMode) {
                            player.sendMessage(`§c⚡ Phase roll failed, roll = ${(roll * 100).toFixed(1)}%, chance = ${(spawnChance * 100).toFixed(1)}%`);
                        }
                    }
                }
            }
...
export function startPhantomSpawner(customConfig) {
    ...
    function tickCheck(playerSlept) {
        if (tickCheckId !== undefined)
            system.clearRun(tickCheckId);
        const time = world.getTimeOfDay();
        let nextInterval = 20;

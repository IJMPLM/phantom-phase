...
const phantomSpawnCounter = new Map();
// Stores last check time to implement 1-2 minute random intervals
const lastSpawnAttemptTime = new Map();
/**
...
function calculatePhantomSpawnChance(daysWithoutSleep) {
    if (daysWithoutSleep < config.minDaysWithoutSleep)
        return 0;
    if (daysWithoutSleep === config.minDaysWithoutSleep)
        return 0; // Day 3 has 0% chance
    if (daysWithoutSleep === config.minDaysWithoutSleep + 1)
        return 25; // Day 4 has 25% chance
    if (daysWithoutSleep === config.minDaysWithoutSleep + 2)
        return 37.5; // Day 5 has 37.5% chance
 ...
 */
export function startPhantomSpawner(customConfig) {
    ...
    const intervalId = system.runInterval(() => {
        ...
        for (const player of players) {
            ...
            if (isNightTime() && isTimeToSpawnAttempt(playerId)) {
                ...
                if (config.debugMode) {
                    player.sendMessage(`§7Phantom Spawn Attempt: ${successfulRoll ? "§aSuccess" : "§cFail"} (${roll.toFixed(1)}/${spawnChance.toFixed(1)}%)`);
                    player.sendMessage(`§7Conditions: Above Sea Level: ${aboveSeaLevel ? "§a✓" : "§c✗"}, Sky Access: ${hasSkyAccess ? "§a✓" : "§c✗"}`);
                }
...
//# sourceMappingURL=phase-spawner.js.map